...
//            cat_sendchar();
/*
https://ru-radio-electr.livejournal.com/1503339.html
http://forum.easyelectronics.ru/viewtopic.php?f=35&t=29697
Бит 17 CMF: флаг соответствия символов
Этот бит устанавливается аппаратно, когда получен символ, определенный как ADD [7: 0]. Это очищается программным обеспечением, записывая 1 в CMCF в регистре USART_ICR.
Прерывание генерируется, если CMIE = 1 в регистре USART_CR1.
0: совпадение символов не обнаружено
1: совпадение символов обнаружено

Бит 4 IDLE: Обнаружена свободная линия
Этот бит устанавливается аппаратно при обнаружении незанятой линии. Прерывание генерируется, если IDLEIE = 1 в регистре USART_CR1. Это очищается программным обеспечением, записывая 1 в IDLECF в регистре USART_ICR.
0: свободная линия не обнаружена
1: свободная линия обнаружена
Примечание. Бит IDLE не будет установлен снова до тех пор, пока не будет установлен бит RXNE (то есть возникнет новая линия ожидания).
Если включен режим отключения звука (MME = 1), IDLE устанавливается, если USART не отключен (RWU = 0), независимо от режима отключения звука, выбранного битом WAKE. Если RWU = 1, IDLE не установлен.

Бит 3 ORE: ошибка переполнения
Этот бит устанавливается аппаратным обеспечением, когда данные, принимаемые в настоящее время в регистре сдвига, готовы к передаче в регистр RDR, пока RXNE = 1. Это очищается программным обеспечением, записывающим 1 в ORECF, в регистре USART_ICR.
...
Примечание. Когда этот бит установлен, содержимое регистра RDR не теряется, а регистр сдвига перезаписывается. Прерывание генерируется, если флаг ORE установлен во время многобуферной связи, если установлен бит EIE.
Этот бит постоянно устанавливается в 0 (без определения превышения), когда бит OVRDIS установлен в регистр USART_CR3.

Бит 1 FE: ошибка кадрирования
Этот бит устанавливается аппаратно при обнаружении десинхронизации, чрезмерного шума или символа прерывания. Это очищается программным обеспечением, записывая 1 в бит FECF в регистре USART_ICR.
В режиме Smartcard, при передаче, этот бит устанавливается, когда максимальное количество попыток передачи достигнуто без успеха (карта NACKs фрейма данных).
Прерывание генерируется, если EIE = 1 в регистре USART_CR1.
0: ошибка кадрирования не обнаружена
1: ошибка кадрирования или обнаружен символ разрыва
*/
